...
export class PointerEvents {
  ...
  constructor(
    ...
    if (!ele) throw new Error('element can not be null');
    if (!pointerDown) throw new Error('pointerDown can not be null');
  ...
  private handleTouchStart(ev: any) {
    if (!this.ele) throw new Error('element can not be null');
    if (!this.pointerDown) throw new Error('pointerDown can not be null');
